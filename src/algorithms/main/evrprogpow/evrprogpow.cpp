...
#include ".../common/ethash/ethash/primes.h"
#include ".../common/ethash/include/keccak.hpp"
#include ".../common/ethash/support/attributes.h"
...
// Verify constants:
static_assert(sizeof(ethash::hash512) == EVRPROGPOW_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(sizeof(ethash::hash1024) == EVRPROGPOW_FULL_DATASET_ITEM_SIZE, "");
static_assert(light_cache_item_size == EVRPROGPOW_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(full_dataset_item_size == EVRPROGPOW_FULL_DATASET_ITEM_SIZE, "");
    ...
    build_light_cache_fn build_fn, int epoch_number, bool full) noexcept
{
    ...
    const size_t full_dataset_size =
        full ? static_cast<size_t>(full_dataset_num_items) * sizeof(ethash::hash1024) :
               evrprogpow_progpow::l1_cache_size;
    ...
    for (uint32_t i = 0; i < evrprogpow_progpow::l1_cache_size / sizeof(full_dataset_2048[0]); ++i)
...
using namespace evrprogpow_main;

extern "C" {

ethash_hash256 evrprogpow_calculate_epoch_seed(int epoch_number) noexcept
{
...
epoch_context* evrprogpow_create_epoch_context(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, false);
}

epoch_context_full* evrprogpow_create_epoch_context_full(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, true);
}

void evrprogpow_destroy_epoch_context_full(epoch_context_full* context) noexcept
{
    evrprogpow_destroy_epoch_context(context);
}

void evrprogpow_destroy_epoch_context(epoch_context* context) noexcept
{
    context->~epoch_context();
    std::free(context);
}

evrprogpow_result evrprogpow_hash(
...
bool evrprogpow_verify_final_hash(const ethash::hash256* header_hash, const ethash::hash256* mix_hash, uint64_t nonce,
...
bool evrprogpow_verify(const epoch_context* context, const ethash::hash256* header_hash,
